...
for i in range(0, len(args)):
    ...
    os.system ("mkdir -p ~/kosmos/deb")
    os.system ("mkdir -p ~/kosmos/logs")
    ...
    os.system("cp -R ~/kosmos/debian-packages/%s/debian/ ." % package)
    ...
    if options.source:
        ...
        os.system("mv -v ../*.dsc ~/kosmos/deb/")
        os.system("mv -v ../*.xz ~/kosmos/deb/")
        os.system("mv -v ../*.changes ~/kosmos/deb/")
    else:
        if options.i386:
            os.system("dpkg-buildpackage -B -ai386 -Zxz -uc -tc 2>&1 | tee build.log")
        else:
            os.system("dpkg-buildpackage -b -Zxz -uc -tc 2>&1 | tee build.log")
        os.system("mv -v build.log ~/kosmos/logs/%s.log" % package)
        os.system("rm -v ../*.changes")
        os.system("mv -v ../*.deb ~/kosmos/deb/")
    os.chdir(os.path.join(os.path.expanduser("~"), "kosmos"))
    os.system("rm -rf /tmp/%s/ " % package)
    print "---------------------------------------------------------------"
